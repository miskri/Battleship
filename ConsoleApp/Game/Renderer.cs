    ...
    public abstract class Renderer {
        ...
        protected string GetLevelDescription(string mode, string currPlayer, string player1Name = "", string player2Name = "") {
            if (mode != "Board Building") {
                return Data.System + $"   Game mode: {mode}, {player1Name} against {player2Name}" +
                       Color.Reset + "\n";
            }

            return Data.System + $"   {currPlayer}, please make up the battle formation for your ships" + Color.Reset + "\n";
        }

        protected string GetFieldCoordinatesTemplate(string mode) {
            string template = "";
            if (mode != "Board Building") {
                template += Data.Default + $"   {Separator}{EmptySpace}{Separator} \n";
                template += $"   {BottomAIndexes}{EmptySpace}{BottomAIndexes} \n";
            }
            else {
                template += Data.Default + $"   {Separator}\n";
        ...
        protected string GetFieldFooter(string mode) {
            ...
            if (mode != "Board Building" && mode != "AI vs AI") {
                fieldFooterString += Data.Default + $"   Enemy field{new string(' ', Separator.Length - 2)}Your field \n";
                fieldFooterString += $"   {Separator}{EmptySpace}{Separator} \n";
            }
            else if (mode == "AI vs AI") {
                fieldFooterString += Data.Default + $"   Beavis field{new string(' ', Separator.Length - 3)}Butthead field \n";
                fieldFooterString += $"   {Separator}{EmptySpace}{Separator} \n";
            }
            else {
                fieldFooterString += Data.Default + $"   {Separator}" + Color.Reset + "\n";
            }
        ...
        protected string GetBattleMenu(IReadOnlyList<string> menuOptions, int row, int rowCount) {
            ...
            for (int z = 0; z < menuOptions.Count; z++) {
                ...
                battleMenu += Data.System + $" {menuOptions[z]}" + Color.Reset + "\n";
            }
