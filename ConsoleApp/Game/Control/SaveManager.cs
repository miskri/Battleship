...
namespace ConsoleApp.Control {
    ...
    public static class SaveManager {
        ...
        public static List<string> GetSavesList() {
            ...
            List<SaveObject> saves = ctx.Saves.ToList();
            List<string> saveNames = new List<string>();
            if (saves.Count > 0) saveNames = saves.Select(save => save.SaveName).ToList();
            saveNames.Reverse();
            return saveNames;
        }
        ...
        public static Save GetSaveReference(string saveName) {
            if (Saves == null) LoadSaves();
            Save dbSave = LoadFromDb(saveName);
            return dbSave?? Saves?.FirstOrDefault(save => save.SaveName == saveName);
        }

        public static int SaveGame(Save save) { return SaveToDb(save); }

        public static void DeleteSavedGame(Save save) {
            if (Saves == null) LoadSaves();
            foreach (Save jsonSave in Saves.Where(jsonSave => jsonSave.SaveName == save.SaveName)) {
                Saves.Remove(jsonSave);
                break;
            }
            SaveChanges();
        ...
        public static void LoadSaves() {
            Saves = new List<Save>();
            StreamReader saveFileReader = new StreamReader(_savePath);
        ...
        private static void SaveChanges() {
            StreamWriter saveFileWriter = new StreamWriter(_savePath);
        ...
        public static bool ContainsThisName(string name) {
            return GetSavesList().Any(save => save == name);
        }
