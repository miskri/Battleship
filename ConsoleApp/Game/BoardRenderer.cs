...
namespace ConsoleApp {
    public class BoardRenderer : Renderer {
        ...
        public void GameOverScreen(GameProperties props, int row) {
            ...
            outputString += GetFieldFooter(props.GameMode);
            outputString += GetBattleHistory(props.BattleHistory);
            outputString += Data.Default + $"   {props.Winner} wins the game! Congratulations!" + Color.Reset + "\n";
            outputString += Data.Default + $"   {Separator}" + Color.Reset + "\n";
        ...
        private string GetFieldsAfterGameEnding(GameProperties props, int index) {
            ...
            if (props.GameMode == "AI vs AI" || props.GameMode == "Player vs Player" && props.Winner == props.Player2Name) {
                ...
                fieldsAfterGame += Data.Default + "      " + Color.Reset;
        ...
        private string GetField(GameProperties props, int row, int col, int index) { // TODO here can cause render mistakes with fields
            ...
            if (props.GameMode != "AI vs AI" && props.GameMode != "Player vs Player") {
                ...
                fieldString += Data.Default + "      " + Color.Reset;
        ...
        private string GetBattleInfo(GameProperties props, int row, int col, string currentPlayer) {
            ...
            battleInfoString += Data.Default + "   Selected quad: ";
            if (row >= 0 && col >= 0 && row < props.FieldSize[0] && col < props.FieldSize[1]) {
                battleInfoString += Data.Alphabet[col] + $"{row + 1}";
            }

            battleInfoString += "\n";
            battleInfoString += Data.Default + $"   This is {currentPlayer} turn! \n";
            battleInfoString += Data.Default + $"   Ships in enemy flotilla: {props.GetEnemyShipsCount()}\n";
            
            if (props.GameMode != "Player vs Player" && props.Manager.TimeManager.GetStepsBackCount(props.Id) > 0) 
                battleInfoString += Data.Default + "   Сharges in the time machine: " +
        ...
        private string DrawEnemyField(string[,] enemyField, int row, int col, int index) {
            ...
            enemyFieldString += Data.Default + (index < 9 ? $" {index + 1} " : $"{index + 1} ") + Color.Reset;
        ...
        private string DrawPlayerField(string[,] playerField, int index) {
            ...
            playerFieldString += Data.Default + (index < 9 ? $" {index + 1} " : $"{index + 1} ") + Color.Reset;
            for (int x = 0; x < playerField.GetLength(1); x++) {
                switch (playerField[index, x]) {
                    case null: case "border":
                        playerFieldString += Data.FieldCell;
                        break;
                    case "ship":
                        playerFieldString += Data.ShipCell;
                        break;
                    case "collision":
                        playerFieldString += Data.CollisionCell;
                        break;
                    case "miss":
                        playerFieldString += Data.MissCell;
                        break;
                }
            }
